...
def get_shifted_images_list(model, ref_image, target_images_list, device):
    model.eval()
    if ref_image is None:
        return []
        
    ref_in = cv2.resize(ref_image, (256, 256)) / 255.0
    inp = torch.tensor(ref_in).permute(2, 0, 1).float().unsqueeze(0).to(device)
    
    with torch.no_grad():
        out = model(inp)
    ref_out = out.squeeze().cpu().numpy().transpose(1, 2, 0)
    shift = np.mean(ref_out - ref_in, axis=(0, 1)) 
    ...
    for img in target_images_list:
        ...
        img_float += shift 
        final_img = (np.clip(img_float, 0, 1) * 255).astype(np.uint8)
...
def processing_img(refpath, targetpath, filename_path):
    ...
    ls_pro = [cropped, target_img]
    results_AI = get_shifted_images_list(model, cropped, ls_pro, device)
    cropped_AI = results_AI[0]
    cropped_AI_img = results_AI[0]
    target_img_AI = results_AI[1]
    target_img_AI_img = results_AI[1]

    result_img, history, orange_history = iterative_color_match(reference_img, cropped_AI)
    ...
    cropped_rgb = cv2.cvtColor(cropped_AI, cv2.COLOR_BGR2RGB)
    target_rgb_AI = cv2.cvtColor(target_img_AI, cv2.COLOR_BGR2RGB)
    cropped_rgb_AI = cv2.cvtColor(cropped_AI_img, cv2.COLOR_BGR2RGB)
    ...
    axes[0,1].set_title("2. Original Target")

    axes[0,2].imshow(cropped_rgb)
    axes[0,2].set_title("3. Detected Crop")

    axes[0,3].imshow(target_rgb_AI)
    axes[0,3].set_title("4. AI Global Shift (Full)")

    axes[1,0].imshow(cropped_rgb_AI)
    axes[1,0].set_title("5. AI Shift (Crop)")

    axes[1,1].imshow(result_rgb)
    axes[1,1].set_title("6. Object Refined (Final)")

    axes[1,2].imshow(full_rgb)
    axes[1,2].set_title("7. Full Image Corrected")

    axes[1,3].plot(history, marker='o', color='orange')
    axes[1,3].set_title("8. Convergence (DeltaE)")
    axes[1,3].set_xlabel("Iteration")
    axes[1,3].set_ylabel("DeltaE")
    axes[1,3].grid(True)

    for i, v in enumerate(history):
        axes[1,3].text(i, v, f"{v:.2f}", fontsize=9, fontweight='bold')
